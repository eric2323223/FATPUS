...
import resources.testscript.Workflow.OnlineCache.OLC_Email_AllDatetypes_DB_Decimal_E2EHelper;
 ...
 * @author min zhao
 */
public class OLC_Email_AllDatetypes_DB_Decimal_E2E extends OLC_Email_AllDatetypes_DB_Decimal_E2EHelper
{
	/**
	 * Script Name   : <b>OLC_Email_AllDatetypes_DB_Decimal_E2E</b>
	 * Generated     : <b>Apr 9, 2012 3:06:15 AM</b>
	 * Description   : Functional Test Script
...
//				GlobalConfig.getRFTProjectRoot()+"/testscript/Workflow/Screens/setup/Alldate.sql");
		...
		mbo.setMboDefinition("sql", "SELECT * FROM sampledb.dba.AllData where type_DECIMAL=:type_DECIMAL");
	//	mbo.setLoadArgument(new LoadArgument().name("type_INT").defaultValue("1"));
		...
		.data("list_items_count=3|"+
				"id=AllData_type_DECIMAL_attribKey,value=2"));
	...
	private CallBackMethod sendNotification() {
		return new CallBackMethod().receiver(WorkFlowEditor.class)
			...
			.parameter(new Email().to("dt").body("<typeInt>2</typeInt>"));
	}
	...
	private CustomJsTestScript customJsTestScripts() {
		...
		s.screen("AllDataDetail").getData("AllData_type_DECIMAL_attribKey");
		return s;
	}
}
