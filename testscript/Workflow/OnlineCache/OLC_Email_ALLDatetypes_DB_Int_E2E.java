...
import resources.testscript.Workflow.OnlineCache.OLC_Email_ALLDatetypes_DB_Int_E2EHelper;
 ...
 * @author min zhao
 */
public class OLC_Email_ALLDatetypes_DB_Int_E2E extends OLC_Email_ALLDatetypes_DB_Int_E2EHelper
{
	/**
	 * Script Name   : <b>OLC_Email_ALLDatetypes_DB_Int_E2E</b>
	 * Generated     : <b>Apr 5, 2012 9:50:01 PM</b>
	 * Description   : Functional Test Script
	...
	public void testMain(Object[] args) 
	{
		...
		mbo.setMboDefinition("sql", "SELECT * FROM sampledb.dba.AllData where type_INT=:type_INT");
	//	mbo.setLoadArgument(new LoadArgument().name("type_INT").defaultValue("1"));
		...
		.data("list_items_count=2|"+
				"id=AllData_type_INT_attribKey,value=1"));
	...
	private CallBackMethod sendNotification() {
		return new CallBackMethod().receiver(WorkFlowEditor.class)
			...
			.parameter(new Email().to("dt").body("<typeInt>1</typeInt>"));
	}
	...
	private CustomJsTestScript customJsTestScripts() {
		...
		s.screen("AllDataDetail").getData("AllData_type_INT_attribKey");
		return s;
	}
}
