...
import resources.testscript.Workflow.Controls.One2One_Ctrl_Slider_E2EHelper;
 ...
 * @author eric
 */
public class One2One_Ctrl_Slider_E2E extends One2One_Ctrl_Slider_E2EHelper
{
	/**
	 * Script Name   : <b>One2One_Ctrl_Slider_E2E_Android</b>
	 * Generated     : <b>Sep 1, 2011 1:29:04 PM</b>
	 * Description   : Functional Test Script
	...
	public void testMain(Object[] args) 
	{
		...
		WN.createWorkFlow(new WorkFlow().startParameter(Cfg.projectName).name(
					"wfOtoslider").option(WorkFlow.SP_SERVER_INIT));
		...
		WorkFlowEditor.addWidget(Cfg.projectName, "wfOtoslider.xbw", "linktoemail", new WFSlider().label("emp_id:")
				.labelPosition("LEFT")
				.newKeyBindMbo("key1,string,Ff_wf_employee,emp_id")
				.maxValue("20")
				.minValue("0"));
		
		WN.createWorkFlowPackage(new WorkFlowPackage()
			.startParameter(WN.wfPath(Cfg.projectName, "wfOtoslider"))
			.unwiredServer("My Unwired Server")
			.assignToUser("dt")
			.verifyResult("Successfully deployed the workflow", true));
		WorkFlowEditor.sendNotification(Cfg.projectName, "wfOtoslider.xbw", new Email()
			.subject("dept=1")
			.to("dt")
			.unwiredServer("My Unwired Server"));
		
//1.used to Android:
		TestResult result = Robot.run("tplan.Workflow.Controls.android.One2One_Ctrl_Slider.Script");
		vpManual("DeviceTest", true, result.isPass()).performTest();
		
//2.used to BB:passed
		TestResult resultBB = Robot.run("tplan.Workflow.Controls.BB.One2One_Ctrl_Slider.Script");
